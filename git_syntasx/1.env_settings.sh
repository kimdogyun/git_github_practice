...
# git의 인증방법 2가지
# 1.웹을 통한 인증(토큰간접발급) 2.직접 토큰 발급

#사용자지정 
git congif --global user.name "유저명"
git congif --global user.email "email명"
...
# 2.로컬에서 이미 만들어진 프로젝트 github에 업로드
# 2-1).git 폴더를 생성
git init
#2-2)원격지 주소를 추가
...
#git add , git commit , git push 작업 실행 
